...
def preprocess_data_from_csv(model_folder, csv_file, input_features, output_features, _binary_feature_names, _numeric_feature_names, _categorical_feature_names, special_feature_names, presaved_mean = None, presaved_stdev = None):
    ...
    df = pd.read_csv(csv_file, nrows = 10000) # REMOVE nrows = 1000 after adjustments
    df.fillna(-1)
    preprocessed_data = []
...
def preprocess_data(
    ...
    df = pd.read_csv(input_folder, nrows=10000)
...
if __name__ == '__main__':
    model_folder = "TEST_models_4_18"  # model folder
    csv_file = "4_18_TEST_models_with_ii.csv"      # csv path
